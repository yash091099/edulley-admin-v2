...
export default function Table({component, columns, data, mapping ,fun , viewDetails }) {
        ...
        <table className="w-full">
          ...
          <tbody>
            {data && data?.map((row, rowIndex) => (
              <tr
                key={rowIndex}
                className={`cursor-pointer h-[5rem] border-y border-[#DBDADE] ${component === 'Student' ? 'tr-row' : ''}`}
                >
                      ...
                      {key === "Name" ? (
                        ...
                        <div className='flex flex-wrap gap-2'>
                          {row["Tags"]?.map((tag, tagIndex) => (
                            <span key={tagIndex} style={{backgroundColor:"#FFF0F0"}} className=" rounded-full px-3 py-1 text-sm font-semibold text-gray-700">
                              {tag}
                            </span>
                          ))}
                        </div>
